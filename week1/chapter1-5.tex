...
By definition 1.6.9 a subsequence $(a_{k_{j}})$ is bounded because $(a_{k})$, Is bounded.

Finally, we can apply Lemma 1.6.14 \marginnote{\begin{lem}Every bounded monotonic sequence in $\mathbb{R}$ converges to an element in $\mathbb{R}$\end{lem}} $(a_{k_{j}}$ is monotonic and bounded and therefore converges to an element in ${R}$.
