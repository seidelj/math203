...
In layman's terms, we delete the all the middle intervals at every stage of constructing the Cantor set.  Which would mean the compliment is dense everyone.
...
Induction step.  
\begin{align*}
\sum_{n=0}^{n+1}z^n &= z^0+z^1+...+z^n + z^{n+1}\\
...
If \marginnote{Ask Jinxin about this problem, I am rusty on Calc.} $0 < p < 1$, case we have $n^{-p} < \epsilon$, which can be rewritten $p > -log_n(e)$ This not possible, because $n \in \mathbb{N}$ and $e$ is going to be some very small non-negative number.
...
We \marginnote{I need help completing this thought} say that the series $\sum_{n=1}^{\infty}\frac{1}{n^p}$ converges if the sequence of partial sums $(S_N)_{N \in \mathbb{N}}$  The series converges if and only if a sequence is a Cauchy sequence.  Which is to say, given $e > 0$, there is a $N_e \in \mathbb{N}$ such that for $N,M > N_e$, (assuming $N>M$), then $|\sum_{k=m+1}^{n}a_n| < e$

\begin{align*}
|S_N - S_M| &< e \\
|S_N - S_M| \leq |(1 + \sum_{n=2}^{N}R_N) - (1 + \sum_{n=2}^{M}R_M)| &< e \\
|\frac{1}{p-1}(1-\frac{1}{N^{p-1}}) - \frac{1}{p-1}(1-\frac{1}{M^{p-1}})| &< e \\
|\frac{1}{p-1}(1-\frac{1}{N^{p-1}} - 1-\frac{1}{M^{p-1}} - 1)| &< e\\
|\frac{1}{p-1}(-\frac{1}{N^{p-1}} - \frac{1}{M^{p-1}})| &< e\\
\end{align*}
